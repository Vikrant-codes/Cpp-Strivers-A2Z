...
// Simulation Approach :- Time Complexity = O(N*M) __ Space Complexity = O(1), where N=no. of words & M=average length of words
/*
...
string mapWordWeights(vector<string>& words, vector<int>& weights) {
    ...
    for (string word : words) {
        // find weight of the current string word
        int weight = 0;
        for (char ch : word)
            weight += weights[ch - 'a'];
        weight %= 26;

        // get the corresponding lowercase alphabet mapped using reverse alphabetical order
        char x = 'z' - weight;
        ans += x;
    }
